...
# Pause zwischen den Messungen des Ultraschall Sensors in Sekunden
sleeptime = 0.8

def Piezo_Buzzer(freq):
    # passiven Piezo-Buzzer mit Frequenz initalisieren
...
def Ultraschall_Sensor():
    ...
    if abstand < 2 or (round(abstand) > 300): # Wenn Abstand zu klein bzw. groß starte Messung erneut
        ...
        time.sleep(5)
        Ultraschall_Sensor()
    else: print("Der Abstand ist: " + str(round(abstand)) + " cm")
try:
    while(True):
